...
const HighRisk = () => {
  ...
  const [popupAtm, setPopupAtm] = useState(null);
  ...
  const fetchAdvancedAtms = () => {
    ...
    fetch(`http://localhost:5000/advanced?${queryParams}`)
      .then((res) => res.json())
      .then((data) => {
      ...
      .then((res) => res.json())
      .then((enrichedData) => {
        setAtmResults(enrichedData);
        setCurrentPage(1);
      })
      .catch((err) => console.error("Advanced ATM fetch or enrich error:", err));
  };

  const getDistrictOptions = () => {
    if (!selectedCity) return [];
    const found = data.find((d) => d.il === selectedCity.value);
    return found ? found.ilceleri.map((ilce) => ({ value: ilce, label: ilce })) : [];
  };

  const getRiskLabel = (risk) => {
    const style = { fontWeight: 'bold' };
    if (risk === 0) return <span style={{ ...style, color: 'green' }}>LOW</span>;
    if (risk === 10) return <span style={{ ...style, color: 'orange' }}>MEDIUM</span>;
    if (risk === 20) return <span style={{ ...style, color: 'red' }}>HIGH</span>;
    return null;
  };

  const paginatedAtms = atmResults.slice(
    (currentPage - 1) * itemsPerPage,
    currentPage * itemsPerPage
  );

  return (
    <div style={{ padding: '2rem', fontFamily: 'Arial', maxWidth: '960px', margin: 'auto' }}>
      <h2 style={{ textAlign: 'center', marginBottom: '2rem' }}>Advanced ATM Risk Search</h2>

      <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', marginBottom: '1.5rem' }}>
        <div style={{ width: '250px' }}>
          <label><b>City</b></label>
          <Select
            options={data.map((d) => ({ value: d.il, label: d.il }))}
            value={selectedCity}
        ...
        <button onClick={fetchAdvancedAtms} style={{ ...buttonStyle, height: '40px', alignSelf: 'flex-end' }}>Search</button>
      </div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', boxShadow: '0 0 8px rgba(0,0,0,0.1)' }}>
        ...
        <tbody>
          {paginatedAtms.map((atm, i) => (
            <tr key={i} style={{ backgroundColor: i % 2 === 0 ? '#fff' : '#f9f9f9' }}>
              <td style={tdStyle}>{atm.name}</td>
              <td style={tdStyle}>{atm.address}</td>
              <td style={tdStyle}>{getRiskLabel(atm.risk_score)}</td>
              <td style={tdStyle}>
             <div style={{ display: 'flex', gap: '0.5rem' }}>
            <button style={buttonStyle}>View in Map</button>
            <button
            style={{ ...buttonStyle, backgroundColor: '#e81123' }}
            onClick={() => setPopupAtm(atm)}
            >
            Risk Detail
            </button>
            </div>
      ...
      {atmResults.length > itemsPerPage && (
        <div style={{ textAlign: 'center', marginTop: '1.5rem' }}>
          ...
          <button
            ...
            onClick={() =>
              setCurrentPage((prev) =>
                prev * itemsPerPage < atmResults.length ? prev + 1 : prev
              )
            }
            disabled={currentPage * itemsPerPage >= atmResults.length}
          >
            Next
          </button>
        </div>
      ...
      {popupAtm && (
        <div style={popupOverlay} onClick={() => setPopupAtm(null)}>
          <div style={popupBox} onClick={(e) => e.stopPropagation()}>
            <h3>{popupAtm.name}</h3>
            <p><b>Address:</b> {popupAtm.address}</p>
            <p><b>Risk:</b> {getRiskLabel(popupAtm.risk_score)}</p>
            <p><b>Detail:</b> {popupAtm.indoor === false
              ? popupAtm.temperature !== null && (popupAtm.temperature <= 5 || popupAtm.temperature >= 32)
                ? 'ATM açık alanda ve sıcaklık uç değerlerde.'
                : 'ATM açık alanda.'
              : 'ATM kapalı alanda.'}
            </p>
            <button style={buttonStyle} onClick={() => setPopupAtm(null)}>Close</button>
          </div>
        </div>
      )}
    </div>
